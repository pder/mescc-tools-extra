...
int read_block_header(struct bunzip_data *bd, struct bwdata *bw)
{
    ...
    for (ii=0; ii<bd->groupCount; ii+=1) bd->mtfSymbol[ii] = ii;
    ...
    for (jj=0; jj<bd->groupCount; jj+=1) {
        ...
        for (ii = 0; ii < symCount; ii+=1) {
            while (1) {
                ...
                if (MAX_HUFCODE_BITS-1 < hh-1) return RETVAL_DATA_ERROR;
...
int start_bunzip(struct bunzip_data **bdp, int src_fd)
{
    ...
    *bdp = malloc(i);
    ...
    char *header = "BZh";
    for (i=0;i<3;i+=1) if (get_bits(bd,8)!=header[i]) return RETVAL_NOT_BZIP_DATA;

    // Next byte ascii 1-9, indicates block size in units of 100k of
    // uncompressed data. Allocate intermediate buffer for block.
    i = get_bits(bd, 8);
    if (i<49 || i>57) return RETVAL_NOT_BZIP_DATA;
